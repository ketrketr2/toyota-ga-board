...
| 09 | **JP活用実績 🟡実測** | 効果ファーストの**活動台帳**型。実装済み活動「2件」（完了ページの2バナー）を主役に、生んだ効果（CV寄与1件・追加滞在11分27秒・回遊×2.1・店舗検索+63%）を最上部へ。**行クリックで「何が変わったか」**（Before/After・ボリューム・効果・実測ログ・次の一手）。母数は1行に格下げ。活動が増えるたび行が増える建て付け |
| 10 | **SNSパフォーマンス 🟡実測** | **バズヒーロー**（精霊馬 8.9万いいね=通常帯81倍・キャプチャ付き）→ **媒体別ボード**（最新投稿のimp・いいね・RP/シェア・コメント＋媒体別分析コメント）→ **ソート可能な投稿一覧**（露出/いいね/RP/コメント/倍率で並び替え・カテゴリフィルタ・行クリックで実画面）→ **カテゴリ×四象限**（露出倍率×反応倍率・対数）で「どの型が跳ねるか」を分析 |
