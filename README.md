...
> ⚠️ SECTOR 00〜07 は **合成デモデータ** で動作（`DEMO DATA` バッジ表示）。**SECTOR 08〜10（OWNED OPS）は実測データ**：公式SNSクロール（2026-08-19）と GA4 実測レポート#007（7/7〜8/5）に基づく。
...
| 08 | **オウンド資産 🟡実測** | JP・SNS・メルマガ・LINEの金額換算を独立集約。総資産 **46.59億円**（STOCK+FLOW×Q）— チャネル行クリックで**計算式・代入値・出典のポップアップ**。実測スナップショット（7/29→8/19）からD/W/M増減換算（+189万円/週）、回遊価値+1.23億円（実測PV/S 3.31）を上振れ計上 |
| 09 | **JP活用実績 🟡実測** | 「活動が生んだ活用 → ボリューム → 質 → 効果」のプレゼン構成。設置面 実測2,429件/28日（試乗予約完了・GA4実測 年換算3.2万件）・toyota.jp全体970万S/3,215万PV・リフト（店舗検索+63%）・成果第1号「1件」・年間価値シナリオ・**実ページキャプチャ対比**・編集できる施策リスト（localStorage） |
| 10 | **SNSパフォーマンス 🟡実測** | 公式SNS実測クロール（X 64.7万・IG 85.1万・YouTube 3ch 127.9万・FB 56万）。投稿ランキングは**媒体別タブ**切替、**投稿クリックで実画面キャプチャをモーダル表示**。刺さる・刺さらない分析／アカウントパワー100点配点マップ／3週間のフォロワー動向 |
